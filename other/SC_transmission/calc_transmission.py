...
def main():
    ...
    filename = '/dirs/data/tirs/downloads/Surfrad/results/analysis_level2_20200802_trans.csv'
    #filename = '/dirs/data/tirs/downloads/Buoy_results/buoy_analysis_level2.csv'
    ...
    # trans10 = np.zeros(len(lat))
    # trans11 = np.zeros(len(lat))
    ratio = np.array(out['ratio'])
    cwv_t = np.array(out['cwv_t'])
    cwv_other = np.array(out['cwv_other'])
    cwv_modtran = np.array(out['cwv_modtran'])
    trans10 = np.array(out['trans10'])
    trans11 = np.array(out['trans11'])
    #cwv_calculated = np.zeros(len(lat))
    ...
    for i in range(len(lat)):
        ...
        if site[i] == 'Goodwin_Creek_MS':
            grnd_alt = 0.098
        elif site[i] == 'Sioux_Falls_SD':
            grnd_alt = 0.473
        elif site[i] == 'Fort_Peck_MT':
            grnd_alt = 0.634
        elif site[i] == 'Desert_Rock_NV':
            grnd_alt = 1.007
        else:
            grnd_alt = 1.890 #km above sea level - Lake Tahoe
        
        tape5_info['lat'] = lat[i]
        tape5_info['lon'] = lon[i]
        ...
        try:
            ...
            date_out = datetime.datetime.strptime(date1+' '+str(hour)+' '+str(minute), '%Y-%m-%d %H %M')
        tape5_info['time'] = hour
        
        
        #if not (date_out.year == 2013 or cwv_modtran[i] != 0):
        if site[i] in ['Sioux_Falls_SD','Fort_Peck_MT','Desert_Rock_NV'] and trans10[i] == 0:
        
            try:
                #pdb.set_trace()
                profile = get_geos.geos_process(date_out, lat[i], lon[i], out_dir,grnd_alt)
                tape5_info['altKm'] = profile[0]
                tape5_info['preshPa'] = profile[1]
                tape5_info['tempK'] = profile[2]
                tape5_info['dewpK'] = profile[3]
                ...
                trans10[i] = np.trapz(list(rsr10 * trans),x=list(wavelength),axis=0)/np.trapz(list(rsr10),x=list(wavelength),axis=0)
                trans11[i] = np.trapz(list(rsr11 * trans),x=list(wavelength),axis=0)/np.trapz(list(rsr11),x=list(wavelength),axis=0)
                ratio[i] = trans11[i]/trans10[i]
            ...
            print(site[i], ' not in list')
            
            
    cwv_ranges = np.array(cwv_modtran)
    cwv_ranges = np.where(np.array(cwv_modtran) < 6, 5, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_modtran) < 5, 4, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_modtran) < 4, 3, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_modtran) < 3, 2, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_modtran) < 2, 1, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_modtran) < 1, 0, cwv_ranges) 
    ...
    out['cwv_modtran'] = cwv_modtran
    out['cwv_ranges'] = cwv_ranges
    out['trans10'] = trans10
    out['trans11'] = trans11
    
    out.to_csv('/dirs/data/tirs/downloads/Surfrad/results/analysis_level2_20200802_trans.csv',index = False)
...
def plot_data():
    filename = '/dirs/data/tirs/downloads/Surfrad/results/analysis_level2_trans1.csv'

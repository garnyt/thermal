...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 21 12:57:25 2021
...
def create_tape5(tape5_info,tape5_name):
    ...
    if 'alt'not in tape5_info:
        tape5_info['alt'] = tape5_info['altKm'][0]+0.0001    
...
def main():
    ...
    grnd_alt = 1.890 #km above sea level  
    ...
    cwv_geos = np.zeros(len(lat))
    cwv_radiosonde = np.zeros(len(lat))
    ...
    for i in range(len(lat)):
        ...
        if date_out.year > 2013:
        
            try:
                        
                # get radiosonde data and plot
                data_out, tape5_info = parse_radiosonde.parse_radiosonde_data(12,date_out.day, month.upper() ,date_out.year,grnd_alt)
                ...
                max_alt = tape5_info['altKm'][-1]
                # get GEOS5 data and remove lower layers
                tape5_geos_info = {}
                tape5_geos_info['lat'] = lat[i]
                tape5_geos_info['lon'] = lon[i]
                profile = get_geos.geos_process(date_out, lat[i], lon[i], out_dir,grnd_alt,max_alt)
                tape5_geos_info['altKm'] = profile[0]
                tape5_geos_info['preshPa'] = profile[1]
                tape5_geos_info['tempK'] = profile[2]
                tape5_geos_info['dewpK'] = profile[3]
                create_tape5(tape5_geos_info,tape5_name)
                runModtran(tape5_name,tape6_name)
                tape6_geos = read_tape6('tape6',filepath='/cis/staff/tkpci/modtran/tape6_geos/')
                
                # if tape6['CWV'] > 1.5:
                #     # plot profiles
                #     plt.figure()  
                #     plt.plot(profile[2],profile[0])
                #     plt.plot(profile[3],profile[0])
                    
                #     plt.plot(tape5_info['tempK'],tape5_info['altKm'])
                #     plt.plot(tape5_info['dewpK'],tape5_info['altKm'])
                #     plt.ylim([grnd_alt,max(tape5_info['altKm'])])
                #     plt.legend(('GEOS5 Temperature','GEOS5 Dewpoint','Radiosonde Temperature','Radiosonde Dewpoint'))
                #     plt.title('GEOS5 CWV: '+ str(tape6_geos['CWV']) + '  Radiosonde CWV: ' + str(tape6['CWV']))
                ...
                cwv_geos[i] = tape6_geos['CWV']
                cwv_radiosonde[i] = tape6['CWV']
                print('GEOS5 CWV: ', tape6_geos['CWV'])
                print('Radiosonde CWV', tape6['CWV'])
                #pdb.set_trace()
                
                
            except:
                print('Oops, someting went wrong...' + date1)
    
    
    cwv_ranges = np.array(cwv_geos)
    cwv_ranges = np.where(np.array(cwv_geos) < 6, 5, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_geos) < 5, 4, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_geos) < 4, 3, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_geos) < 3, 2, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_geos) < 2, 1, cwv_ranges) 
    cwv_ranges = np.where(np.array(cwv_geos) < 1, 0, cwv_ranges) 
        
    cwv_geos = list(cwv_geos)
    cwv_radiosonde = list(cwv_radiosonde)
    cwv_ranges = list(cwv_ranges)
    
    
    
    out['cwv_goes'] = cwv_geos
    out['cwv_radiosonde'] = cwv_radiosonde
    #out['cwv_ranges_'] = cwv_ranges

...
import pdb
import save_L8AsterInClass_SW_calc
import estimate_landsat_emissivity_SW_calc
import calc_SW
...
def download_landsat(filepath, scene_ID):
    ...
    #https://earthexplorer.usgs.gov/download/5e83d0b84df8d8c2/LC80430332020295LGN00/EE/
...
def appTemp(radiance, band='b10'):
    ...
    if band == 'b10':
        K2 = 1320.6539 # new derived coefficients K2 = 1321.0789
        K1 = 775.1682  # new derived coefficients K1 = 774.8853
    elif band == 'b11':
        K2 = 1200.8253  # new derived coefficients K2 = 1201.1442
        K1 = 481.1861  # new derived coefficients K1 = 480.8883
    else:
        ...
        with requests.Session() as session:
            ...
            for i in range(lat_tiles):
                for j in range(lon_tiles):
                    ...
                    filename = folder + 'downloads/' + file 
...
def main():
    ...
    filepath = '/dirs/data/tirs/downloads/DRS/'
    #filepath = '/dirs/data/tirs/downloads/Surfrad_level2_downloads/Surfrad_P016_R032/'
    
    # create folder for ASTER downloaded data 
    filepathASTER = '/dirs/data/tirs/downloads/DRS/ASTER/'
    #filepathASTER = '/dirs/data/tirs/downloads/aster/E13_E14_NDVI_files/'
    
    # add scene ID to download - other format does not work
    #scene_ID = 'LC80390372020299LGN00'
    scene_ID = 'LC80410352020297LGN00'
    #scene_ID = 'LC80430332020295LGN00'
    ...
    # open all files needed for emissivity calculations in one class
    dataOut, wkt_projection, geoTransform = save_L8AsterInClass_SW_calc.dataForEmisCalc(filepathLandsat, sceneID)
    ls_emis_final_b10 = estimate_landsat_emissivity_SW_calc.estimate_landsat_emissivity(dataOut,MTL_file, sensor='TIRS10', convert='yes')
    ls_emis_final_b11 = estimate_landsat_emissivity_SW_calc.estimate_landsat_emissivity(dataOut,MTL_file,sensor='TIRS11',convert='no')
   
       
    # calculate SW
    #pdb.set_trace()
    splitwindowLST, appTemp_b10, appTemp_b11 = calcSW(dataOut.getRad('rad10'), dataOut.getRad('rad11'), ls_emis_final_b10, ls_emis_final_b11)
    
    # save temperature data to georegistered tif file
    saveGeotif(splitwindowLST, filepathLandsat, sceneID +'_SW_LST.TIF', wkt_projection, geoTransform)
    saveGeotif(ls_emis_final_b10, filepathLandsat, sceneID +'_emis10.TIF', wkt_projection, geoTransform)
    saveGeotif(ls_emis_final_b11, filepathLandsat, sceneID +'_emis11.TIF', wkt_projection, geoTransform)               
    
    return splitwindowLST,  ls_emis_final_b10   
    

    
if __name__ == '__main__':
    splitwindowLST , ls_emis_final_b10 = main()
    
plt.subplot(1,2,1)
plt.imshow(splitwindowLST, vmin = 300, vmax = 310)
plt.colorbar()
plt.axis('off')

plt.subplot(1,2,2)
plt.imshow(ls_emis_final_b10, vmin = 0.85, vmax = 0.99)
plt.colorbar()
plt.axis('off')
